...
export function TechnologyAutocomplete({
    ...
    useEffect(() => {
        ...
        setLoading(true);
        const id = setTimeout(() => {
            apiFetch<{ data: TechnologyOption[] }>(
                `/technologies/search?q=${encodeURIComponent(term)}`,
            )
                .then((r) => {
                    if (!abort) {
                        setOptions(r.data);
        ...
        <div className="relative" ref={containerRef}>
            ...
            {open && (options.length > 0 || loading) && (
                <div className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded border border-neutral-300 bg-white shadow dark:border-neutral-600 dark:bg-neutral-800">
                    ...
                    {!loading &&
                        options
                            .filter((o) => !disabledIds.includes(o.id))
                            .map((o) => (
                                <button
                                    key={o.id}
                                    type="button"
                                ...
                                >
                                    ...
                                    <span>{o.name}</span>
                                    <span className="ml-auto text-xs tracking-wide text-neutral-400 uppercase">
                                        {o.category}
                                    </span>
                                </button>
                            ))}
                    {!loading &&
                        options.filter((o) => !disabledIds.includes(o.id))
                            .length === 0 && (
                            <div className="px-3 py-2 text-xs text-neutral-500">
                                Sin resultados
                            </div>
