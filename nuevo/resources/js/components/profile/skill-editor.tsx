...
export function SkillEditor({ initialSkills }: Props) {
    ...
    const [skills, setSkills] = useState<Skill[]>(() =>
        [...serverSkills].sort((a, b) => a.position - b.position),
    );
    ...
    const addTech = async (tech: Technology) => {
        ...
        router.post(
            '/skills',
            { technology_id: tech.id, level: 'learning' },
            {
                preserveScroll: true,
                only: ['skills'],
                onError: (errs) => {
    ...
    const updateSkill = (id: number, payload: Partial<Skill>) => {
        setError(undefined);
        router.patch(`/skills/${id}`, payload as any, {
            // cast para evitar conflicto de tipos con Inertia
            preserveScroll: true,
            only: ['skills'],
    ...
    const onDragOver = (id: number) => (e: React.DragEvent) => {
        ...
        setSkills((prev) => {
            const dragged = prev.find((s) => s.id === dragId.current)!;
            const others = prev.filter((s) => s.id !== dragged.id);
            const index = others.findIndex((s) => s.id === id);
            const newArr = [
        ...
        <div className="space-y-3">
            <div className="flex items-center gap-2">
                <TechnologyAutocomplete
                    onSelect={addTech}
                    disabledIds={skills.map((s) => s.technology.id)}
                />
                {adding && (
            ...
            <InputError
                message={
                    error || (usePage() as any)?.props?.errors?.technology_id
                }
            />
