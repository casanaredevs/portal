...
interface Props { initialProfiles: ExternalProfile[]; }

const PLATFORMS = [
  'github','linkedin','twitter','stackoverflow','youtube','devto','medium','hashnode','facebook','instagram','twitch','threads','discord','mastodon','gitlab','bitbucket'
];
...
export function ExternalProfilesEditor({ initialProfiles }: Props) {
  ...
  const serverProfiles = (page.props as any).externalProfiles || initialProfiles;
  const [profiles, setProfiles] = useState<ExternalProfile[]>(() => [...serverProfiles].sort((a,b)=>a.position-b.position));
  ...
  useEffect(() => {
    ...
    setProfiles(prev => {
      const incoming = [...serverProfiles].sort((a,b)=>a.position-b.position);
      return incoming.map(p => dirtyLabelsRef.current[p.id] !== undefined ? (prev.find(x=>x.id===p.id) || p) : p);
    });
  }, [serverProfiles.map(p=>p.id+'-'+p.position+'-'+p.handle+'-'+(p.label||'')).join('|')]);

  // Limpieza timers al desmontar
  useEffect(()=> () => { Object.values(saveTimersRef.current).forEach(clearTimeout); }, []);

  const createProfile = () => {
    if (!form.handle.trim()) { setError('Handle requerido'); return; }
    setAdding(true); setError(undefined);
    router.post('/external-profiles', { platform: form.platform, handle: form.handle }, {
      preserveScroll: true,
  ...
  const onDragOver = (id:number) => (e:React.DragEvent) => { e.preventDefault(); if(dragId.current===id) return; setProfiles(prev => {
    const dragged = prev.find(p=>p.id===dragId.current)!; const others = prev.filter(p=>p.id!==dragged.id);
    const index = others.findIndex(p=>p.id===id); const newArr = [...others.slice(0,index), dragged, ...others.slice(index)];
    return newArr.map((p,i)=> ({...p, position:i+1}));
  }); };
  const onDrop = () => {
    const ordered = profiles.slice().sort((a,b)=>a.position-b.position).map(p=>p.id);
    router.post('/external-profiles/reorder', { external_profile_ids: ordered }, {
      preserveScroll: true,
      only: ['externalProfiles'],
      onError: errs => setError(Object.values(errs)[0] as string),
  ...
  const scheduleLabelSave = (id:number) => {
    ...
    saveTimersRef.current[id] = setTimeout(() => {
      ...
      if (pending !== undefined) {
        ...
        router.patch(`/external-profiles/${id}`, { label: pending }, {
          preserveScroll: true,
          only: ['externalProfiles'],
          onError: errs => setError(Object.values(errs)[0] as string)
        });
  ...
  const onLabelChange = (id:number, value:string) => {
    setProfiles(prev => prev.map(p => p.id===id ? { ...p, label: value } : p));
  ...
  return (
    <div className="space-y-3">
      ...
      <ul className="space-y-2">
        {profiles.sort((a,b)=> a.position - b.position).map(p => (
          <li key={p.id}
              draggable
            ...
            <a href={p.url} target="_blank" className="truncate text-blue-600 hover:underline dark:text-blue-400" rel="noreferrer">{p.handle}</a>
            <input
            ...
            />
            {p.is_verified && <span className="rounded bg-green-600 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-white">verificado</span>}
            <button type="button" onClick={()=> removeProfile(p.id)} className="ml-auto rounded px-2 py-1 text-xs text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20">Eliminar</button>
          </li>
        ))}
      </ul>
    </div>
