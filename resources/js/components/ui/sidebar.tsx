      ...
      <div
        className={cn(
          ...
          variant === "floating" || variant === "inset"
            ? "group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)+(--spacing(4))+0.125rem)]"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      tabIndex={-1}
      onClick={(e) => {
        // Prevent accidental toggles when the rail overlays menu items: only act if the rail itself was the direct click target
...
function SidebarMenuButton({
  ...
  const button = (
    <Comp
      ...
      onClick={(e) => {
        if (asChild) {
          // Evita que el clic burbujee hasta elementos que puedan togglear el sidebar
          e.stopPropagation()
        }
        props.onClick?.(e)
      }}
